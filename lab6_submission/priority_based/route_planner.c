...
// Edge structure
typedef struct Edge { //edited to add time
    int to;
    double weight;
    double time; 
    struct Edge* next;
...
void add_edge(Graph* g, int from, int to, double weight, double time) {
...
// A* algorithm
void astar(Graph* g, int start_idx, int end_idx, double* dist, int* prev, double* time_taken, int* nodes_explored) {
    PriorityQueue pq;
    pq_init(&pq);
    ...
    *nodes_explored = 0;
    ...
    while (!pq_empty(&pq)) {
        ...
        while (edge != NULL) { //while current node HAS at least one adjacency:
            ...
            double alt = dist[u] + edge->weight; //calculate alternate distance to node v than what is currently known
...
int main(int argc, char* argv[]) {
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        if (sscanf(line, "%d,%d,%lf,%lf", &from, &to, &distance, &time) == 4) { //EDITED TO ADD TIME
            int from_idx = find_node_index(&g, from);
            int to_idx = find_node_index(&g, to);
            if (from_idx != -1 && to_idx != -1) {
                add_edge(&g, from_idx, to_idx, distance, time);
            }
        }
    }
    fclose(fp);
    ...
    if (strcmp(algorithm, "dijkstra") == 0) {
        ...
        printf("=== A* Algorithm ===\n");
        astar(&g, start_idx, end_idx, dist, prev, time_taken &nodes_explored);
